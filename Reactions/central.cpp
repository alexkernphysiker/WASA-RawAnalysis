	...
	res.Trigger(tn).post()<<(make_shared<ChainCheck>()
	    ...
	    <<(make_shared<ChainOr>()		
			...
			<<[measured_eta_angle](WTrack&T)->bool{return measured_eta_angle(T)<40;}
                        ...
                        <<[ggggggdiff](WTrack&T)->bool{return ggggggdiff(T)<0.10;}
			...
			<< make_shared<Hist1D>("He3nCentralGammas6","TIM5",he3ggggggimdiff)
                        ...
                        <<[ggggggmm](WTrack&T)->bool{return (ggggggmm(T)>2.70)&&(ggggggmm(T)<3.00);}
	...
	res.Trigger(tn).post()<<(make_shared<ChainCheck>()
		...
		<< ( make_shared<ChainCheck>()
			...
			<< ( make_shared<ChainCheck>()
				...
				<<[measured_eta_angle](WTrack&T)->bool{return measured_eta_angle(T)<60;}
				...
				<<[ggmm](WTrack&T)->bool{return (ggmm(T)>2.70)&&(ggmm(T)<3.00);}
