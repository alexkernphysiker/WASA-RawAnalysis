...
void Search2Gamma(Analysis&res){
	...
	res.Trigger(tn).post()<< ( make_shared<ChainCheck>()
	    << make_shared<Hist1D>("CentralGammas","GammaCount2",Axis([]()->double{return registered.size();},-0.5,9.5,10))
...
void Search6Gamma(Analysis&res){
	...
	static point<double> pi0_triple_mm(INFINITY,INFINITY);
	res.Trigger(tn).post()<< ( make_shared<ChainCheck>()
		<< make_shared<Hist1D>("CentralGammas","GammaCount6",Axis([]()->double{return registered.size();},-0.5,9.5,10))
		<< []()->bool{return registered.size()>0;}
		<< []()->bool{AcceptedE=0;return true;}
